...
const TECH = [
  { name:'MongoDB',     icon:'🍃', color:'text-green-400  bg-green-400/10  border-green-400/20'  },
  { name:'Express.js',  icon:'⚡', color:'text-gray-300   bg-gray-400/10   border-gray-400/20'   },
  { name:'React 18',    icon:'⚛️', color:'text-cyan-400   bg-cyan-400/10   border-cyan-400/20'   },
  { name:'Node.js',     icon:'🟢', color:'text-lime-400   bg-lime-400/10   border-lime-400/20'   },
  { name:'Tailwind CSS',icon:'🎨', color:'text-sky-400    bg-sky-400/10    border-sky-400/20'    },
  { name:'JWT Auth',    icon:'🔐', color:'text-purple-400 bg-purple-400/10 border-purple-400/20' },
  { name:'Recharts',    icon:'📊', color:'text-indigo-400 bg-indigo-400/10 border-indigo-400/20' },
  { name:'Vite',        icon:'⚡', color:'text-yellow-400 bg-yellow-400/10 border-yellow-400/20' },
];
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <section className="pt-32 pb-20 px-4 sm:px-6 text-center relative overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto">
          ...
          <h1 className="animate-slide-up delay-100 text-5xl md:text-6xl font-black text-white mb-6 leading-tight">
            Built with purpose,<br />
            <span className="shimmer-text">for real people.</span>
          </h1>
          <p className="animate-slide-up delay-200 text-xl text-gray-500 leading-relaxed">
      ...
      <section className="py-16 px-4 sm:px-6 max-w-6xl mx-auto">
        <div className="reveal grid grid-cols-1 lg:grid-cols-5 gap-0 rounded-3xl overflow-hidden border border-white/5 shadow-2xl">
          ...
          {/* Right: Message */}
          <div className="lg:col-span-3 bg-gray-900 p-10 lg:p-12">
            ...
            <div className="flex flex-wrap gap-3 mt-8">
              {[
                ...
                <div key={b.text} className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                  <span>{b.icon}</span>
                  <span className="text-xs font-medium text-gray-300">{b.text}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-20 px-4 sm:px-6 max-w-4xl mx-auto">
        <div className="text-center mb-12 reveal">
          <span className="text-indigo-400 text-sm font-bold uppercase tracking-widest">How It Was Built</span>
          <h2 className="text-4xl font-black text-white mt-3">The build timeline</h2>
        </div>
          ...
          <div className="space-y-8">
            {TIMELINE.map((t, i) => (
              <div key={t.year} className={`reveal delay-${(i+1)*100} flex gap-6 items-start`}>
                ...
                <div className="bg-gray-900 rounded-2xl p-5 border border-white/5 flex-1">
      ...
      <section className="py-20 px-4 sm:px-6 max-w-6xl mx-auto">
        <div className="text-center mb-12 reveal">
          <span className="text-indigo-400 text-sm font-bold uppercase tracking-widest">What We Stand For</span>
          <h2 className="text-4xl font-black text-white mt-3">Our values</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {VALUES.map((v, i) => (
            <div key={v.title} className={`reveal delay-${(i%2+1)*100} flex gap-4 p-6 bg-gray-900 rounded-2xl border border-white/5 hover:border-indigo-500/20 transition-all card-hover`}>
      ...
      <section className="py-20 px-4 sm:px-6" style={{ background:'linear-gradient(180deg,rgba(99,102,241,0.04) 0%,transparent 100%)' }}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12 reveal">
            <span className="text-indigo-400 text-sm font-bold uppercase tracking-widest">Technology</span>
            <h2 className="text-4xl font-black text-white mt-3">Built with modern tools</h2>
          </div>
      ...
      <section className="py-20 px-4 text-center">
        <div className="max-w-2xl mx-auto reveal">
          ...
          <p className="text-gray-500 mb-8 text-lg">Join hundreds of users who track their money smarter with SpendWise.</p>
          <Link to="/register"
